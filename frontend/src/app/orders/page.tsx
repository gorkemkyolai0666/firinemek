...
import { formatCurrency, formatDate, getStatusBadgeClass, getOrderStatusLabel, getProductCategoryLabel } from '@/lib/utils';
              ...
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <div>
                  ...
                  <div className="text-sm mt-1 flex flex-wrap gap-2 items-center" style={{ color: 'var(--text-muted)' }}>
                    <span>{order.quantityUnits} adet</span>
                    <span>·</span>
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  {order.status === 'confirmed' && (
                    <button onClick={() => handleStatusUpdate(order.id, 'baking')} className="btn-primary text-xs px-3 py-1">Fırına Al</button>
                  )}
                  {order.status === 'baking' && (
                    <button onClick={() => handleStatusUpdate(order.id, 'packed')} className="btn-primary text-xs px-3 py-1">Paketle</button>
                  )}
                  {order.status === 'packed' && (
                    <button onClick={() => handleStatusUpdate(order.id, 'delivered')} className="btn-primary text-xs px-3 py-1">Teslim Et</button>
                  )}
                </div>
              </div>
            </div>
