...
export default function RegisterPage() {
  ...
  const [form, setForm] = useState({
    email: '', password: '', firstName: '', lastName: '', bakeryName: '', city: '',
  });
      ...
      <div className="card w-full max-w-md">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <span className="font-display text-xl font-bold" style={{ color: 'var(--text-primary)' }}>FirinEmek</span>
              <p className="text-[10px] tracking-widest uppercase" style={{ color: 'var(--text-muted)' }}>Üretim Atölyesi</p>
            </div>
          </div>
          <h1 className="font-display text-3xl font-bold" style={{ color: 'var(--text-primary)' }}>Kayıt Ol</h1>
          <p className="text-sm mt-2" style={{ color: 'var(--text-muted)' }}>Fırın üretim atölyeniz için yeni hesap oluşturun</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input type="email" value={form.email} onChange={(e) => update('email', e.target.value)} className="input-field" placeholder="ornek@üretim.com" required />
          </div>
          ...
          <div>
            <label className="block text-sm font-medium mb-1.5" style={{ color: 'var(--text-muted)' }}>Üretim Atölyesi Adı</label>
            <input type="text" value={form.bakeryName} onChange={(e) => update('bakeryName', e.target.value)} className="input-field" placeholder="Örn: Kadıköy Üretim" required />
          </div>
