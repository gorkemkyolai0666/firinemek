...
interface DashboardStats {
  ...
  weekProductionBatches: number;
  pendingOrders: number;
  readyOrders: number;
  lowStockIngredients: number;
  recentCustomers?: Customer[];
  upcomingProductionBatches?: ProductionBatch[];
}
  ...
  return (
    <BottomNavLayout>
      <div className="mb-8">
        ...
        <p className="mt-1" style={{ color: 'var(--text-muted)' }}>Üretim atölyenizin güncel durumu</p>
      </div>
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-6 gap-4 mb-8">
            ...
            <div className="stat-card">
              <span className="text-sm font-medium" style={{ color: 'var(--text-muted)' }}>Bugünkü Üretim</span>
              <span className="text-3xl font-bold" style={{ color: 'var(--accent)' }}>{stats.todayProductionBatches}</span>
            </div>
            <div className="stat-card">
              <span className="text-sm font-medium" style={{ color: 'var(--text-muted)' }}>Haftalık Üretim</span>
              <span className="text-3xl font-bold" style={{ color: 'var(--accent)' }}>{stats.weekProductionBatches}</span>
            </div>
            <div className="stat-card">
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            <div className="card">
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-display text-lg font-bold" style={{ color: 'var(--text-primary)' }}>Yaklaşan Üretim Partileri</h2>
                <Link href="/production-batches" className="text-sm font-medium" style={{ color: 'var(--accent)' }}>Tümü</Link>
              </div>
              {!stats.upcomingProductionBatches?.length ? (
                <p className="text-sm py-4 text-center" style={{ color: 'var(--text-muted)' }}>Yaklaşan productionBatch seansı bulunmuyor</p>
              ) : (
                <div className="space-y-3">
                  {stats.upcomingProductionBatches.map((productionBatch) => (
                    <div key={productionBatch.id} className="flex items-center justify-between p-3 rounded-lg border" style={{ borderColor: 'var(--border-color)' }}>
                      <div>
                        <div className="font-medium text-sm" style={{ color: 'var(--text-primary)' }}>
                          {productionBatch.customer?.companyName || '—'}
                        </div>
                        <div className="text-xs" style={{ color: 'var(--text-muted)' }}>
                          {productionBatch.customer?.contactName && `${productionBatch.customer.contactName} · `}
                          {getProductionBatchTypeLabel(productionBatch.type)}
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="text-xs font-medium" style={{ color: 'var(--text-muted)' }}>{formatDateTime(productionBatch.date)}</div>
