...
import { LayoutDashboard, Users, BookOpen, Wheat, ShoppingBag, ChefHat, Moon, Sun, LogOut } from 'lucide-react';

const navItems = [
  { href: '/dashboard', label: 'Panel', icon: LayoutDashboard },
  { href: '/customers', label: 'Müşteriler', icon: Users },
  { href: '/recipes', label: 'Tarifler', icon: BookOpen },
  { href: '/ingredients', label: 'Hammadde', icon: Wheat },
  { href: '/orders', label: 'Siparişler', icon: ShoppingBag },
  { href: '/production-batches', label: 'Üretim', icon: ChefHat },
];
      ...
      >
        <div className="p-6 border-b" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-display text-lg font-bold" style={{ color: 'var(--text-nav)' }}>FirinEmek</div>
        ...
        <div className="p-4 border-t space-y-2" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleLogout} className="flex-1 flex items-center justify-center gap-1 py-2 rounded-lg text-xs" style={{ color: 'var(--danger)' }}>
      ...
      <main className="flex-1 md:ml-64">
        ...
        >
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center font-display font-bold text-sm" style={{ background: 'var(--copper)', color: 'white' }}>F</div>
            <span className="font-display font-bold text-sm" style={{ color: 'var(--text-nav)' }}>FirinEmek</span>
          </div>
