...
API_URL="${API_URL:-http://localhost:4540/api}"
PASS=0
FAIL=0
...
echo "=== FirinEmek Integration Tests ==="
echo "API: $API_URL"
echo ""
...
LOGIN_RESPONSE=$(curl -s -w "\n%{http_code}" "$API_URL/auth/login" \
  ...
  -d '{"email":"demo@kadikoyfirin.com","password":"demo123456"}')
...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "$API_URL/production-batches" -H "Authorization: Bearer $TOKEN")
assert_status "List Production Batches" 200 "$HTTP_CODE"
...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "$API_URL/recipes" -H "Authorization: Bearer $TOKEN")
assert_status "List Recipes" 200 "$HTTP_CODE"
...
CREATE_CUSTOMER=$(curl -s -w "\n%{http_code}" "$API_URL/customers" \
  ...
  -H 'Content-Type: application/json' \
  -d '{"companyName":"Test Fırın Evi","contactName":"Deniz Koç","phone":"0544 999 88 77","city":"İstanbul"}')
...
if [ -n "$CUSTOMER_ID" ]; then
  ...
  CREATE_BATCH=$(curl -s -w "\n%{http_code}" "$API_URL/production-batches" \
    -H "Authorization: Bearer $TOKEN" \
    -H 'Content-Type: application/json' \
    -d "{\"date\":\"2027-02-15T10:00:00Z\",\"type\":\"trial\",\"customerId\":\"$CUSTOMER_ID\"}")
  BATCH_CODE=$(echo "$CREATE_BATCH" | tail -1)
  assert_status "Create Production Batch" 201 "$BATCH_CODE"

  BATCH_ID=$(echo "$CREATE_BATCH" | sed '$d' | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])" 2>/dev/null || echo "")

  if [ -n "$BATCH_ID" ]; then
    HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "$API_URL/production-batches/$BATCH_ID" \
      -H "Authorization: Bearer $TOKEN" \
      -X DELETE)
    assert_status "Delete Production Batch" 200 "$HTTP_CODE"
  fi
